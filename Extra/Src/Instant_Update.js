module.exports = async function() {
    ...
    const { body } = await got('https://raw.githubusercontent.com/dongdev06/Global_Fca_DongDev/main/InstantAction.json'); 
    ...
    const LocalVersion = require('../../package.json').version;
        if (Number(LocalVersion.replace(/\./g,"")) < Number(json.Version.replace(/\./g,"")) ) {
            ...
            try {
                execSync(`npm install fca-mdongdev@${json.Version}`, { stdio: 'inherit' });
                ...
                try {
                    ...
                    await new Promise(resolve => setTimeout(resolve, 3000));
                    execSync(`npm install fca-mdongdev@${json.Version} --force`, { stdio: 'inherit' });
                    log.info("〈 FCA-UPDATE 〉➜","Update Complete, Restarting...");
                    await new Promise(resolve => setTimeout(resolve, 3000));
                    ...
                    try {
                        ...
                        fs.rmdirSync((process.cwd() + "/node_modules/fca-mdongdev" || __dirname + '../../../fca-mdongdev'), { recursive: true });
                        await new Promise(resolve => setTimeout(resolve, 3000));
                        execSync(`npm install fca-mdongdev@${json.Version}`, { stdio: 'inherit' });
                        log.info("〈 FCA-UPDATE 〉➜","Update Complete, Restarting...");
                        await new Promise(resolve => setTimeout(resolve, 3000));
                        ...
                        await new Promise(resolve => setTimeout(resolve, 3000));
                        log.warn("〈 FCA-UPDATE 〉➜","Please contact to owner about update failed and screentshot error log at Fb.com/Donqdz.profile.206");
                        await new Promise(resolve => setTimeout(resolve, 3000));
                        process.exit(1);
                    }
                }
